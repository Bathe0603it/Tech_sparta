...
class Menu_model extends CI_Model {
         ...
         { 
                      ...
                      $this->db->select('m.id,m.name, m.created_date, m.state, u.fullname as created_by, m.level, m.parent, md.title as module');
                      $this->db->from('menu as m');
                      $this->db->join('user as u', 'u.id = m.created_by');
                      $this->db->join('module as md','md.name = m.module','left');
                      if( $get['module'] != null ) 
                      $this->db->where('m.module',$get['module']);
                      if($get['key'] != null )
                      $this->db->like('m.name',$get['key']); 
         ...
         { 
                      $offset = ( (int)$get['page'] - 1 ) * $limit;
                      $this->db->select('m.id,m.name, m.created_date, m.state, u.fullname as created_by, p.level, p.id as position_id, p.parent,md.title as module');
                      $this->db->from('menu as m');
                      $this->db->join('user as u', 'u.id = m.created_by');
                      $this->db->join('position as p', 'p.menu_id = m.id');
                      $this->db->join('module as md','md.name = m.module','left');
