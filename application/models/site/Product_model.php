<?php
    ...
    class Product_model extends CI_Model
    {
        ...
        {
            $this->load->database();
            $this->orders   = array('orders','DESC');
        }
    
        public function Db_list($menu_id , $limit , $offset)
        {
            $query = $this->db->select('product.*')
                                    ->from('product')
                                    ->join('menu_product','product.id = menu_product.product_id')
                                    ->where('menu_id',$menu_id)
                                    ->where('state', 1)
                                    ->order_by($this->orders[0],$this->orders[1])
                                    ->limit($limit,$offset)
                                    ->get();
            return $query->result_array();
        }
        
        public function Db_list_count($menu_id)
        {
            $query = $this->db->select('product.*')
                                    ->from('product')
                                    ->join('menu_product','product.id = menu_product.product_id')
        ...
        {
            $this->db->select('id,parent,name,slug,avatar,giaban,giakhuyenmai,summary, hidden_price');
        ...
        public function getProductFeatureInCategory($category = null){
