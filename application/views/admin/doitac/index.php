...
<article>
    ...
    <header class="header_control">
        <label>Quản lý đội ngũ tư vấn</label>
        <ul>
            <li><a href="<?php echo base_url('admin/'.$this->router->fetch_class().'/add');?>" class="button">Thêm</a></li>
            <li><a href="#" class="button" onclick="submit_form('<?php echo base_url('admin/'.$this->router->fetch_class().'/delete');?>');">Xóa</a></li>
        </ul>
    </header>
    <div class="fillter">
        <form name="fillter" action="" method="get">
            <ul>
                ...
                <li><a href="<?php echo base_url('admin/'.$this->router->fetch_class().'/index');?>" class="button">Hủy</a></li>
            </ul>
        </form>
    </div>
    <form name="adminForm" id="adminForm" action="" method="post">
        <table class="table">
            ...
            <tbody>
                <?php if( !empty($list) ):
                    foreach( $list as $val ):
                    $edit_link = base_url('admin/'.$this->router->fetch_class().'/edit?id='.$val['id'].'&page='.$get['page']);
                    $dir_img = MEDIAPATH.'/images/'.$this->router->fetch_class().'/'.$val['image'];
                    $img = is_file( $dir_img ) ? base_url('media/images/'.$this->router->fetch_class().'/_thumbs/Images/'.$val['image']) : base_url('templates/admin/images/no-image.png');
                    ?>
                <tr>
