...
<article>
    ...
    <header class="header_control">
        ...
        <ul>
            <li><a href="<?php echo base_url('admin/'.get_name_class().'/add');?>" class="button">Thêm</a></li>
            <li><a href="#" class="button" onclick="submit_form('<?php echo base_url('admin/'.get_name_class().'/delete');?>');">Xóa</a></li>
        </ul>
    </header>
    <div class="fillter">
        <form name="fillter" action="" method="get">
            <ul>
                ...
                <li><a href="<?php echo base_url('admin/'.get_name_class().'/index');?>" class="button">Hủy</a></li>
            </ul>
        </form>
    </div>
        ...
        <table class="table">
            <thead>
                <tr>
                    ...
                    <th><span>Tiêu đề sự kiện</span></th>
            ...
            <tbody>
                <?php if( !empty($list) ):
                    foreach( $list as $val ):
                    $edit_link = base_url('admin/'.get_name_class().'/edit?id='.$val['id'].'&page='.$get['page']);
                    $edit_name = $val['state'] == 0 ? $val['name'].'<i class="nhap">(bản nháp)</i>' : $val['name']; 
                    $parent_link = base_url( 'admin/'.get_name_class().'/index?category='.$val['parent'] );
                    ?>
                <tr>
